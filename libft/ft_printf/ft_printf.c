...
int	check_s(char *str)
{
	...
	i = 0;
	if (!str)
		i = ft_putstr_ffd("(null)", 1);
	else
		i = ft_putstr_ffd(str, 1);
	return (i);
}
...
int	print_var(va_list str, const char	*str1, int i)
{
	...
	if (str1[i] == 's')
		...
		ret = ft_putchar_ffd(va_arg(str, int), 1);
	else if (str1[i] == 'd' || str1[i] == 'i')
		ret = ft_putnbr_ffd(va_arg(str, int), 1);
	else if (str1[i] == 'x')
